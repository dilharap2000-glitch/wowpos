...
// CORS and Request Headers for Serverless / Cloud Hosting
app.use((req: Request, res: Response, next: NextFunction) => {
...
app.post('/api/members', requireAuth, requireGymTenant, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const created = await GymService.createMember({
      ...
      partnerName: memberType === 'couple' ? partnerName : undefined,
      partnerPhone: memberType === 'couple' ? partnerPhone : undefined,
      partnerMemberNumber: memberType === 'couple' ? partnerMemberNumber : undefined,
...
// Lookup member by code/barcode/number/id
app.get('/api/members/lookup/:code', requireAuth, requireGymTenant, async (req: AuthRequest, res: Response) => {
  try {
    const businessId = req.businessId || resolveBusinessId(req.gymId);
...
// Fallback JSON 404 Handler for API endpoints
